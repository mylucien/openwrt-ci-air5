...
#define CONFIG_BOOTCOMMAND  "bootspinor"
#define CONFIG_BOOTARGS "console=ttyMSM0,115200n8 loglevel=4 rootwait"
...
#endif
/*
 * Air5 (IPQ8072A + W25R512NWEIQ 64MB) specific additions
 * SPI-NOR OpenWrt boot via bootspinor command
 *
 * Flash layout:
 *   0x000000 ~ 0x310000   启动链     3.06MB  (不动)
...
/* Enable bootspinor command */
#define CONFIG_CMD_BOOTSPINOR

/* Partition map (must match MIBIB) */
#define CONFIG_AIR5_KERNEL_OFFSET    0x00400000
#define CONFIG_AIR5_KERNEL_SIZE      0x00800000   /* 8MB  */
#define CONFIG_AIR5_ROOTFS_OFFSET    0x00C00000
#define CONFIG_AIR5_ROOTFS_SIZE      0x03000000   /* 48MB */
#define CONFIG_AIR5_OVERLAY_OFFSET   0x03C00000
#define CONFIG_AIR5_OVERLAY_SIZE     0x00400000   /* 4MB  */

/* ENV in APPSBLENV partition (0x1F0000, confirmed from smeminfo) */
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"fdt_high=0x4A200000\0" \
	"fit_config=1\0" \
	"bootargs_base=console=ttyMSM0,115200n8 loglevel=4 rootwait\0" \
	"bootargs_extra=\0" \
	/* partition offsets */ \
	"kernel_offset=0x400000\0" \
	"kernel_size=0x800000\0" \
	"rootfs_offset=0xC00000\0" \
	"rootfs_size=0x3000000\0" \
	"overlay_offset=0x3C00000\0" \
	/* write kernel FIT to flash (called by sysupgrade) */ \
	"flash_kernel=" \
		"sf probe 0 && " \
		"sf erase ${kernel_offset} ${kernel_size} && " \
		"sf write ${loadaddr} ${kernel_offset} ${filesize}\0" \
	/* write rootfs squashfs to flash */ \
	"flash_rootfs=" \
		"sf probe 0 && " \
		"sf erase ${rootfs_offset} ${rootfs_size} && " \
		"sf write ${loadaddr} ${rootfs_offset} ${filesize}\0" \
	/* TFTP rescue: re-flash kernel then reboot */ \
	"tftp_rescue=" \
		"tftpboot ${loadaddr} openwrt-air5-kernel.itb && " \
		"run flash_kernel && " \
		"reset\0"
